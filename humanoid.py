...
class Humanoid:
            ...
            if hasattr(obj, meta_data_prop):
                ...
                if "last" not in meta_data_prop:
                    algorithms.print_log_report("WARNING",
                                                "metadata {0}.{1} not found".format(obj.name, meta_data_prop))
        self.metadata_realtime_activated = True
    ...
    def generate_character(self, random_value, prv_face, prv_body, prv_mass, prv_tone, prv_height, prv_phenotype,
        ...
        body_keys = ["Armpit", "Elbows", "Chest", "Body", "Arms", "Feet", "Wrists", "Waist", "Torso", "Stomach",
                     "Shoulders", "Pelvis", "Neck", "Legs", "Hands"]
                ...
                for tr_parameter in tr_data:
                    if tr_parameter[0] in prop:
                        linear_factor = tr_parameter[1] * transformation_1 + tr_parameter[2] * transformation_2 - \
                                        tr_parameter[1] * last_transformation_1 - tr_parameter[
                                            2] * last_transformation_2
                            ...
                            if modifier.name == modifier_name:
                                for prop in modifier.get_properties():
                                    ...
                                    if mix:
                                        ...
                                        self.character_data[prop] = self.search_best_value(measure_name, wish_measure,
                                                                                           modifier, prop)
                                self.combine_morphings(modifier)
    ...
    def load_character(self, data_source, reset_string="nothing", reset_unassigned=True, mix=False,
        ...
        if "manuellab_vers" in charac_data:
            if not algorithms.check_version(charac_data["manuellab_vers"]):
                algorithms.print_log_report("WARNING",
                                            "{0} created with vers. {1}. Current vers is {2}".format(log_msg_type,
                                                                                                     charac_data[
                                                                                                         "manuellab_vers"],
                                                                                                     self.lab_vers))
        else:
    ...
    def load_measures(self, filepath):
        ...
        if not ("measures" in char_data):
            algorithms.print_log_report("ERROR", "This json has not the measures info, {0}".format(
