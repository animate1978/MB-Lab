...
class ProxyEngine:
    ...
    def fit_proxy_object(self, proxy_offset=0.0, proxy_threshold=0.5, create_proxy_mask=False, transfer_w=True):
        ...
        if status == "OK":
            ...
            correct_smooth_mod = algorithms.new_modifier(proxy, self.corrective_modifier_name, 'CORRECTIVE_SMOOTH',
                                                         parameters)
            ...
            for polyg in obj.data.polygons:
                ...
                if max_deform > threshold:
                    for idx in polyg.vertices:
                        ...
                        shape_to_finish.data[
                            idx].co = corrected_position  # + fitted_forma.vertices[idx].normal*difference.length
